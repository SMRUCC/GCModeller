...
Imports SMRUCC.genomics.Data.STRING.StringDB.WebAPI
...
Here, we describe the update to version 9.1 of STRING, introducing several improvements: 
      ...
      PubMed:=23203871)>
<Package("stringdb.web_api", Description:="STRING has an application programming interface (API) which enables you to get the data without using the 
graphical user interface of the web page. The API is convenient if you need to programmatically access some 
information but still do not want to download the entire dataset. There are several scenarios when it is 
practical to use it. For example, you might need to access some interaction from your own scripts or want to 
incorporate some information in STRING to a web page.
...
Public Module stringWebApi

    ''' <summary>
    ''' Main entry point of STRING
    ''' </summary>
    ''' <remarks></remarks>
    Public ReadOnly Property StringDb As New Database("string-db.org/api")
    ''' <summary>
    ''' Alternative entry point of STRING
    ''' </summary>
    ''' <remarks></remarks>
    Public ReadOnly Property StringEMBL As New Database("string.embl.de/api")
    ''' <summary>
    ''' The sister database of STRING
    ''' </summary>
    ''' <remarks></remarks>
    Public ReadOnly Property StitchEMBL As New Database("stitch.embl.de/api")
    ...
    ''' 
    <ExportAPI("downloads.gene_ids.interaction")>
    ...
    ''' <returns></returns>
    ''' <remarks>
    ''' http://string-db.org/api/psi-mi/interactions?identifier=gene_id
    ''' </remarks>
    <ExportAPI("downloads.genomics.interactions")>
    Public Function DownloadInteractions(PTT As PTT,
                                         <Parameter("Dir.Export", "Default directory is current work directory.")>
                                         Optional EXPORT As String = "./") As Integer

        Dim lstID As String() = PTT.GeneIDList.ToArray
        Return stringWebApi.DownloadInteractions(lstID, EXPORT)
    End Function
End Module
