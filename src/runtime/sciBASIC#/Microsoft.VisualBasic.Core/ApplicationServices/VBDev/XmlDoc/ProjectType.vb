...
Namespace ApplicationServices.Development.XmlDoc.Assembly
    ...
    Public Class ProjectType
        ...
        Friend Sub New(t1 As ProjectType, t2 As ProjectType)
            ...
            fields = (t1.fields.Values.AsList + t2.fields.Values).GroupBy(Function(f) f.Name.ToLower).ToDictionary(Function(g) g.nodes, Function(g) g.Sum(Me))
            events = (t1.events.Values.AsList + t2.events.Values).GroupBy(Function(f) f.Name.ToLower).ToDictionary(Function(g) g.nodes, Function(g) g.Sum(Me))
            properties = (t1.properties.Values.AsList + t2.properties.Values).IteratesALL.GroupBy(Function(f) f.Name.ToLower).ToDictionary(Function(g) g.nodes, Function(g) g.ToList)
            methods = (t1.methods.Values.AsList + t2.methods.Values).IteratesALL.GroupBy(Function(f) f.Name.ToLower).ToDictionary(Function(g) g.nodes, Function(g) g.ToList)
        End Sub
