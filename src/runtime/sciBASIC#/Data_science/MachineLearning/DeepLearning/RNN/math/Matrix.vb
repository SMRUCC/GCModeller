	...
	Public Class Matrix
		...
		Private Sub New(data As Double()())
			...
			Catch e As Exception
				Throw New ArgumentException("MxN array expected, where M != 0 and N != 0.", e)
			End Try
		End Sub
		...
		Private Sub New(M As Integer, data As Double())
			If Not M > 0 Then
				Throw New ArgumentException("M > 0 expected as a matrix dimension.")
			End If
			If Not (data IsNot Nothing AndAlso data.Length <> 0 AndAlso data.Length Mod M = 0) Then
		...
		Public Shared Function zeros(M As Integer, N As Integer) As Matrix
			If Not (M > 0 AndAlso N > 0) Then
				Throw New ArgumentException("M > 0 and N > 0 expected as matrix dimensions.")
			End If
		...
		Public Shared Function zeros(k As Integer) As Matrix
			If Not k > 0 Then
				Throw New ArgumentException("k > 0 expected for a k-dimensional vector.")
			End If
		...
		Public Shared Function zerosLike(other As Matrix) As Matrix
			If other Is Nothing Then
				Throw New NullReferenceException("Non-null Matrix expected as a shape template.")
			End If
		...
		Public Shared Function oneHot(k As Integer, i As Integer) As Matrix
			If Not (0 <= i AndAlso i < k) Then
				Throw New ArgumentException("0 <= i < k expected to create a one hot k-dimensional vector.")
			End If
		...
		Public Overridable Function clip(x_a As Double, x_b As Double) As Matrix
			If Not x_a < x_b Then
				Throw New ArgumentException("An interval [a,b], a < b expected.")
			End If
		...
		Public Overridable Function oneHotIndex() As Integer
			If Not (M = 1 OrElse N = 1) Then
				Throw New Exception("A vector expected to be able to return the one-hot index.")
			End If
		...
		Public Overridable Function at(i As Integer) As Double
			...
			If Not (i >= 0 AndAlso If(M = 1, i < N, i < M)) Then
				Throw New IndexOutOfRangeException("Vector element index out of bounds.")
			End If
		...
		Public Overridable Function at(i As Integer, j As Integer) As Double
			If Not (0 <= i AndAlso i < M AndAlso 0 <= j AndAlso j < N) Then
		...
		Public Overridable Function at(index As Matrix) As Double
			If index Is Nothing Then
				Throw New NullReferenceException("Non-null Matrix expected as a one hot index.")
			End If
