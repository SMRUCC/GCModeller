...
```bash
cd test
dotnet run -- selftest     # 内置自检（T1–T11）
dotnet run -- demo         # 8 个演示问题（背包 / 选址 / 指派 / 混合整数 / 不可行 / 无界 / 大规模背包）
```

自检覆盖：已知最优对拍、与**暴力枚举**逐实例对拍（20 个随机纯整数实例）、
整数间隙问题（LP 21 → 整数 20）、不可行与无界证书、预处理开关一致性、
割平面实际生成（分数根松弛）与全幺模对照（根松弛天然整数、不应产生割）、
以及全部模型的可行性 / 整数性 / 目标值对账。

演示中的 30 件 0/1 背包可见完整流水线效果：根松弛 307.552 → 16 条割 → 19 个节点
→ 整数最优 303.99（相对间隙 0）。
...
| 分支规则 | most-fractional / first-fractional / pseudo-cost 选项已留出，当前实现为 most-fractional |
