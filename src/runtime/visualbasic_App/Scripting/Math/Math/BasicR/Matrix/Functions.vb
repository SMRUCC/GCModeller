#Region "2cde4ff5640490fab9c3e82bd3970101, ..\Math\BasicR\Matrix\Functions.vb"
...
Namespace BasicR
    ...
    Partial Class MATRIX
        ...
        ''' </remarks>
        Public Shared Function SG(K As Matrix, Return_M As Matrix, Return_N As Matrix) As Int16
        ...
        ''' <remarks></remarks>
        Public Shared Function Pinv(K As Matrix, Return_K As Matrix) As Int16
            Dim n As Int16 = K.GetSize
            Dim rank As Int16 = GetRank(K, 9)
            Dim m As Int16 = K.Length / n
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function GetRank(K As Matrix, error_ As Int16) As Int16
            Dim n As Int16 = K.GetSize
            Dim m As Int16 = K.Length \ n
            Dim i As Int16 = 0
            Dim i1 As Int16
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function EigenValue(K11 As Matrix, n As Int16, LoopNumber As Int16, Errro As Int16, Ret As Matrix, IsHess As Boolean) As Boolean 'ret里是n*2的数组，第一列是实数部分，第2列为虚数部分
        ...
        ''' <remarks></remarks>
        Public Shared Function Hessenberg(A As Matrix, n As Int16, ret As Matrix) As Int16 '
            Dim i As Int16
            Dim j As Int16
            Dim k As Int16
            Dim temp As Double
        ...
        ''' <remarks></remarks>
        Public Shared Function Det2(k As Matrix, N As Integer) As Double '求矩阵的行列式。K的数组大小为N*N的,不然程序出错.这个适合N比较大的方阵
            N -= 1
            Dim l(N, N) As Double
            Dim u(N, N) As Double
        ...
        ''' <remarks></remarks>
        Public Shared Function SPD(K As Matrix) As Int16 '返回-1 矩阵非对称矩阵，返回0矩阵不正定，返回1矩阵正定.主要是判断矩阵是否是正定矩阵
            Dim n As Int16 = K.GetSize
            Dim m As Int16 = K.Length / n
            If m <> n Then
                Return -1
        ...
        ''' <remarks></remarks>
        Public Shared Function LLt(A As Matrix, L As Matrix, is1_是否已经正定 As Boolean) As Boolean '。其中A=L*(L的转置)
            '用平方根法分解A
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function QR22(A As Matrix, Q As Matrix, R As Matrix, Q_n As Int16, R_n As Int16) As Boolean '此函数不像Math_Matrix_QR2函数一样行数不小于列数,所以应该通用.
        ...
        ''' <remarks></remarks>
        Public Shared Function LU(K As Matrix, n As Int16, L As Matrix, U As Matrix) As Boolean '方阵的LU分解
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function Inv2(K As Matrix, Return_K As Matrix, N As Integer) As Boolean
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function DetF(k As Matrix, N As Integer) As Double '
            Dim i As Integer
            If N = 1 Then
                Return k(0, 0)
            End If
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function Inv(K As Matrix, Return_K As Matrix) As Boolean '求矩阵K的逆.成功返回True与其逆矩阵Return_K
        ...
        ''' <remarks></remarks>
        Public Shared Function Svd(A As Matrix, m As Int16, Ret As Matrix) As Int16 '返回矩阵A的奇异值Ret。本函数出错返回-1。成功返回奇异值的个数，即m*1矩阵的Ret的m
            Dim n As Int16 = A.Length / m
        ...
        Public Shared Function SymTridMatrix(A As Matrix, n As Int16, Is对称 As Boolean, ret As Matrix) As Boolean '用豪斯赫尔蒙德变换将实对称阵化为对称三对角阵，翻译徐士良老师的解法
            Dim m As Int16 = A.Length / n
        ...
        ''' </remarks>
        Public Shared Function Pow(A As Matrix, m As Integer, n As Integer, Ret As Matrix) As Boolean '求m*m方阵的n次方。注意,没有采用特征值法。为原始的2个矩阵相乘
            m -= 1
            Dim i As Integer
            Dim j As Integer
            Dim k As Integer
        ...
        ''' </remarks>
        Public Shared Function PolyRoots2(A As Matrix, A_n As Integer, LoopNumber As Int16, Erro As Integer, Ret As Matrix) As Integer '失败返回0。成功返回Ret行数
        ...
        ''' </remarks>
        Public Shared Function Cond(k As Matrix, m As Integer) As Double
            '返回矩阵的2范数
            Dim s(0, 0) As Double
            Dim i As Integer
            Dim n As Integer
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function Scatter(X As Matrix, m As Integer, S As Matrix) As Integer
        ...
        Public Shared Function PolyDivEx(A1 As Matrix, A2 As Matrix, RetMod As Matrix, Ret As Matrix, Erro As Integer) As Integer '多项式的除法，里边的数组均为1*n的矩阵,原理:A1/A2=Ret……RetMod'函数最终返回Ret商的数组大小
        ...
        '''    1.00000000000000 ]
        ''' </remarks>
        Public Shared Sub EigTorF(A1 As Matrix, A_m As Integer, EigValve As Double, X As Matrix)
            '采用全选主元素法求解
...
A:              For temp_i = A_m + 1 To n
        ...
        Public Shared Function EigSym(A As Matrix, n As Int16, Erro1 As Int16, Ret As Matrix, Ret_Eigenvectors As Matrix) As Boolean '返回n阶对称矩阵K的特征值ret.翻译徐士良老师的算法。其方法是雅可比过关法。特征向量Ret_Eigenvectors每一列对应ret每一行的特征值
...
Loopexit:
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function Mul(K1 As Matrix, K2 As Matrix, n As Integer, Return_K As Matrix) As Boolean '矩阵相乘K1为m*n,K2为n*l,Return_K为m*l
        ...
        ''' </remarks>
        Public Shared Function Sqrt(K As Matrix, n As Integer, ks As Matrix) As Int16
            '求方阵K的平方根
            'n表示K的阶数
            'ks返回的平方根
            '函数成功返回1
        ...
        ''' </remarks>
        Public Shared Function RU(F As Matrix, n As Integer, R As Matrix, U As Matrix) As Boolean
            '正定、非奇异方阵F进行右极分解
            'n为F的阶数
            '其中F=R*U
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function Hamiltonian(k As Matrix, m As Integer, ret As Matrix) As Int16
        ...
        ''' <returns></returns>
        ''' <remarks>A1/A2=Ret……RetMod</remarks>
        Public Shared Function PolyDiv(A1 As Matrix, A2 As Matrix, RetMod As Matrix, Ret As Matrix, Erro As Integer) As Integer '多项式的除法，里边的数组均为1*n的矩阵,原理:A1/A2=Ret……RetMod'函数最终返回Ret商的数组大小
            Dim n1 As Integer = A1.Length
            Dim n2 As Integer = A2.Length
            Dim N As Integer
            Dim i As Integer
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Shared Function PolyGCFCall(A1 As Matrix, A1_n As Integer, A2 As Matrix, A2_n As Integer, Ret As Matrix, Erro As Integer) As Integer
            If A1_n < A2_n Then
