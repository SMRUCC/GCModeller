...
Writes a SBML Level 3 Version 1 file.
...
sub writeSBML{
    ...
    my $output = new IO::File(">".$output_file."_3-1.sbml");
...
sub writeListOfReactions{
    ...
    foreach my $reac_alias (keys(%reac)){
        ...
        $writer->emptyTag(  "speciesReference",
                            "species" => $reac{$reac_alias}, # delete this line?
    ...
    if ($reaction_ref->{'aliasReacLink'}){
       foreach my $aliasRL (keys(%{$reaction_ref->{'aliasReacLink'}})){
           if(defined($reaction_ref->{'stoichiometry'}->{$aliasRL})){
            ...
            $writer->emptyTag(  "speciesReference",
                                "species" => $reaction_ref->{'aliasReacLink'}->{$aliasRL},# delete this line?
                                "stoichiometry" => $stoich,
                                "constant" => "true");
            $writer->characters("\n");
        }
