...
Namespace AnalysisAPI
    ...
    Public Module TSSsAnalysis
        ...
        Public Function GenerateDoorOperon(Operons As Operon(), PTT As PTT) As DOOR
            ...
            Dim DoorOperons = DOOR.Reload(LQuery)
            ...
            DoorOperons = DOOR.Reload(Merged.ToArray)

            Return DoorOperons
        End Function

        Public Function GenerateDoorOperon(Idx As String, Gene As GeneBrief) As Operon
            Dim Genes = {GenerateDoorGene(Gene)}
            Dim DoorOperon As Operon =
              New Assembly.DOOR.Operon(Idx, Genes)
            Return DoorOperon
        End Function

        Public Function GenerateDoorOperon(idx As String, Operon As Operon, PTT As PTT) As Operon
            Dim setValue = New SetValue(Of OperonGene) <= NameOf(OperonGene.OperonID)
            Dim Genes = (From ID As String
                         In Operon.Genes
                         Select setValue(GenerateDoorGene(ID, PTT), value:=idx)).ToArray
            Dim DoorOperon As Operon = New Operon(idx, Genes)
            Return DoorOperon
        End Function

        Private Function GenerateDoorGene(gene As GeneBrief) As OperonGene
            Dim DoorGene As New OperonGene With {
...
EXIT_:      Return DoorGene
        ...
        Private Function GenerateDoorGene(ID As String, PTT As PTT) As OperonGene
            ...
            If Gene Is Nothing Then
                Return New OperonGene With {.Synonym = ID}
            Else
        ...
        Public Function SubstituteID(Operon As Operon, PTT As PTT) As Operon
        ...
        Public Function ParsingTSSs(data As Transcripts(), Genome As FastaSeq) As FastaFile
            ...
            Dim Fasta = (From Site As Transcripts
                         In LQuery
                         Let FastaSeq = Site.GetTSSLoci(Parser)
                         Where Not FastaSeq Is Nothing AndAlso FastaSeq.Length >= 6
                         Select FastaSeq).ToArray
        ...
        Private Function InternalTrimHead(ByRef Fasta As FastaSeq()) As FastaFile
            Dim TrimHead = (From obj In Fasta.AsParallel Select ID = obj.Title(0), obj Group By ID Into Group).ToArray
            For Each item In TrimHead
                Dim dat = item.Group.ToArray
                If dat.Count > 1 Then
                    For i As Integer = 0 To dat.Count - 1
                        dat(i).obj.SetAttribute(0, dat(i).obj.Title(0) & "-" & i + 1)
                    Next
                End If
            Next

            For Each obj In Fasta
                obj.SetAttribute(0, obj.Title(0).Replace(" ", "_"))
            Next

            Dim File = CType(Fasta, FastaFile)
            Return File
        End Function
        ...
        Public Function DifferentTSSs(condition1 As Transcripts(),
                                      ...
                                      KEGG As bGetObject.Pathway()) As TSSsDifferent()
            ...
            Dim Diff = (From Paired In LQuery.AsParallel
                        ...
                        Select New TSSsDifferent With {
                            ...
                            .Pathway = (From KO As bGetObject.Pathway
                                        In KEGG
