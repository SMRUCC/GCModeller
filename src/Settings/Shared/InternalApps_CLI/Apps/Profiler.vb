...
''' A genome background data file which is created by ``/KO.clusters`` or ``/GO.clusters`` tools.
''' </param>
''' <param name="cluster_id">
''' A list of specific cluster id that used for program debug use only.
''' </param>
''' <param name="format">
''' apply this argument to specify the output table format, by default is in GCModeller table format, or you can assign the ``KOBAS`` format value at this parameter.
''' </param>
''' <param name="out">
''' The file path of the result output, the output result table format is affects by the ``/format`` argument.
''' </param>
''' <param name="geneSet">
''' A text file that contains the gene id list that will be apply the GSEA analysis.
''' </param>
''' <param name="hide_progress">
''' A logical flag argument that controls the console screen display the progress bar or not.
''' </param>
...
'''
''' <param name="[in]">
''' A repository directory that contains the pathway map data and which is generated by ``/Download.Pathway.Maps`` tools in cli app &apos;KEGG_tools&apos;.
''' </param>
...
'''
''' <param name="uniprot">
''' Uniprot database that contains the uniprot_id to KO_id mapping.
''' </param>
''' <param name="maps">
''' This argument should be a directory path which this folder contains multiple KEGG reference pathway map xml files. A xml file path of the kegg pathway map database is also accepted!
''' </param>
