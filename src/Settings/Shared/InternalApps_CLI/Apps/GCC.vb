...
''' </param>
''' <param name="[in]">
''' The genome annotation data in genbank format, apply for the genome data modelling which target genome is not yet published to public.
''' </param>
''' <param name="maps">
''' The KEGG reference pathway data repository, not the data repository for Map render data.
''' </param>
''' <param name="location_as_locus_tag">
''' If the target genome for create the VirtualCell model is not yet publish on NCBI, 
...
'''
''' <param name="kegg">
''' A directory path that contains pathway data from command ``kegg_tools /Download.Pathway.Maps``.
''' </param>
''' <param name="[in]">
''' A NCBI genbank file that contains the genomics data. If the genome contains multiple replicon like plasmids, 
...
'''
''' <param name="pathway">
''' Apply a pathway module filter on the network model, only the gene contains in the given pathway list then will be output to user. 
