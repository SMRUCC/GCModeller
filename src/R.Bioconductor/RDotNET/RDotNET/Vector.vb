...
Public MustInherit Class Vector(Of T)
    ...
    Default Public MustOverride Property Item(index As Integer) As T
    ...
    Default Public Overridable Property Item(name As String) As T
        Get
...
#End Region
    ...
    Public Sub CopyTo(destination As T(), length As Integer, Optional sourceIndex As Integer = 0, Optional destinationIndex As Integer = 0)
        ...
        While Interlocked.Decrement(length) >= 0
            destination(Math.Max(Interlocked.Increment(destinationIndex), destinationIndex - 1)) = Me(Math.Max(Interlocked.Increment(sourceIndex), sourceIndex - 1))
        End While
    End Sub
