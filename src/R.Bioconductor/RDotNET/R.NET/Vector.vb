...
Public MustInherit Class Vector(Of T)
	...
	''' <param name="engine">The <see cref="REngine"/> handling this instance.</param>
	''' <param name="type">The element type.</param>
    ''' <param name="vector__1">The elements of vector.</param>
	Protected Sub New(engine As REngine, type As SymbolicExpressionType, vector__1 As IEnumerable(Of T))
		MyBase.New(engine, engine.GetFunction(Of Rf_allocVector)()(type, vector__1.Count()))
		SetVector(vector__1.ToArray())
	End Sub
	...
	Private Function getIndex(name As String) As Integer
		...
		End If
		Dim names__1 As String() = Names
		If names__1 Is Nothing Then
			Throw New NotSupportedException("The vector has no names defined - indexing it by name cannot be supported")
		End If
		Dim index As Integer = Array.IndexOf(names__1, name)
		Return index
	End Function
	...
	Public ReadOnly Property Names() As String()
		Get
			...
			Dim names__1 As SymbolicExpression = GetAttribute(namesSymbol)
			If names__1 Is Nothing Then
				Return Nothing
			End If
			Dim namesVector As CharacterVector = names__1.AsCharacter()
	...
	Public Sub CopyTo(destination As T(), length__1 As Integer, Optional sourceIndex As Integer = 0, Optional destinationIndex As Integer = 0)
		...
		While System.Threading.Interlocked.Decrement(length__1) >= 0
