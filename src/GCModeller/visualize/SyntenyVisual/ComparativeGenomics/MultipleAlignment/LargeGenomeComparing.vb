#Region "Microsoft.VisualBasic::cbc260498ff113cd7008eee9b8494e82, visualize\visualizeTools\ComparativeGenomics\MultipleAlignment\LargeGenomeComparing.vb"
...
Namespace ComparativeAlignment
    ...
    Module LargeGenomeComparing
        ...
        Private Function __drawing(models As ComparativeGenomics.GenomeModel,
            ...
            gdi.Graphics.FillRectangle(New SolidBrush(Color.FromArgb(200, Color.Black)), rect)
            models.genes = models.genes.OrderBy(Function(g) g.Left).ToArray

            Dim preLeft As Integer = 0
            Dim LegendHeightOffset As Integer = 20
            Dim LegendSize As SizeF = gdi.Graphics.MeasureString(models.SegmentOffset, Font)
            Dim locusFont As New Font(FontFace.MicrosoftYaHei, sizeOfLocus)
            Dim loci As New Point(RegionLeft - 0.5 * LegendSize.Width, height + LegendHeightOffset)

            If DisplayRule Then
                gdi.Graphics.DrawString(models.SegmentOffset, Font, Brushes.Black, loci)
            End If
            ...
            For i As Integer = 0 To models.Count - 1   '绘制基本图形
                ...
                If gene.Color Is Nothing Then
                    Call gdi.Graphics.FillPath(Brushes.BlueViolet, TrModel)
                Else
                    Call gdi.Graphics.FillPath(gene.Color, TrModel)
                End If
            ...
            LegendSize = gdi.Graphics.MeasureString(preLeft, Font)
        ...
        <ExportAPI("Invoke.Drawing",
                   Info:="If the parameter color_overrides is not null then all of the gene color will overrides the cog color as the parameter specific color.")>
        Public Function InvokeDrawing(model As DrawingModel,
                                      <Parameter("GeneID.Disp")> Optional DispGeneID As Boolean = True,
                                      <Parameter("Rule.Disp")> Optional DisplayRule As Boolean = True) As Image
