...
Module Program
    ...
    Private Sub RunPipeline(config As RunConfig)
        ...
        Dim profile = PhyleticProfile.FromHits(config.SampleId, pfamHits)
        Console.WriteLine($"  样本ID: {profile.SampleId}")
        ...
        For Each kv In models
            Console.WriteLine($"    - [{kv.Key}] {kv.Value.PhenotypeInfo.Accession} ({kv.Value.PhenotypeInfo.Category})")
        ...
        predictor.VotingCommitteeSize = 5
        ...
        For Each r In results
            If r.FinalPrediction = 1 Then
                ...
                Dim keyFeatures = explainer.ExtractKeyFeatures(models(r.PhenotypeId), profile)
                For Each f In keyFeatures
                    Dim present = If(f.IsPresent, "[*]", "[ ]")
                    Console.WriteLine($"    {present} {f.PfamAcc,-12} class={f.Class} PCC={f.PearsonCorrelation:F3} " &
        ...
        For Each r In present
            Console.WriteLine($"    -> [{r.PhenotypeId}] {r.PhenotypeName} ({r.PhenotypeCategory})")
        Next
        Console.WriteLine()
    End Sub
    ...
    Private Sub SaveResults(results As List(Of PhenotypePredictionResult),
        ...
        Using writer As New StreamWriter(summaryFile)
            ...
            For Each r In results
                ...
                writer.WriteLine($"{r.PhenotypeId,-12}{vbTab}{r.PhenotypeName,-35}{vbTab}{r.PhenotypeCategory,-15}{vbTab}{pred,-10}{vbTab}{r.PositiveVotes}/{r.TotalVotes}{vbTab}{r.AverageScore:F6}")
            Next
        End Using
            ...
            For Each r In results
                If r.FinalPrediction = 1 Then
                    writer.WriteLine(explainer.GenerateReport(r.PhenotypeId, r.PhenotypeName,
                                                              explainer.ExtractKeyFeatures(models(r.PhenotypeId), profile)))
                    writer.WriteLine()
                End If
            Next
        End Sub
