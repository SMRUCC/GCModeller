...
1474-7596 (Linking)",
    ...
    Public Module DESeq
        ...
        Public Function DiffGeneCOGs(result As String, PTT As PTT, Optional log2Fold As Double = 2, Optional IdenticalLog2Folds As Double = 1) As DESeqCOGs()
            Dim DiffCsv = DocumentStream.File.Load(result)
        ...
        Public Function DESeq2(<Parameter("dataExpr0",
                               ...
                               sampleTable As DocumentStream.File,
        ...
        Private Function __runR(RScript As String, directory As String) As DocumentStream.File
            ...
            '替换回基因编号
            Dim ID As String() = DocumentStream.File.Load(directory & "\countData.csv").Column(0).Skip(1).ToArray
            Dim DiffCsv As DocumentStream.File = DocumentStream.File.Load(DiffPath)

            DiffCsv(0)(1) = "locus_tag"  ' Gene -> locus_tag

            For i As Integer = 1 To ID.Length
                Dim row As DocumentStream.RowObject = DiffCsv(i)
                row(1) = ID(i - 1)
            Next
        ...
        Public Function DESeq2(<Parameter("Count.Data",
            ...
            Dim colDataMAT As New DocumentStream.File    ' For matrix input: a DataFrame or data.frame with at least a single column. Rows of colData correspond to columns of countData.
            Dim HeadRow = New DocumentStream.RowObject From {""}
            ...
            Dim colData As String = $"{FileIO.FileSystem.GetParentPath(countData)}/{IO.Path.GetFileNameWithoutExtension(countData)}.colData.csv"
            ...
            Dim ScriptPath As String = $"{FileIO.FileSystem.GetParentPath(countData)}/{IO.Path.GetFileNameWithoutExtension(countData)}.R.DESeq.txt"
            ...
            '替换回基因编号
            Dim ID = DocumentStream.File.Load(countData).Column(0).Skip(1).ToArray
            Dim DiffCsv = DocumentStream.File.Load(DiffPath)
        ...
        Public Function HTSeqCount(SAM As String, GFF As String, Optional PairedEnd As Boolean = True) As Boolean
            ...
            If PairedEnd Then
                ...
                Dim GFF_ID As String = IO.Path.GetFileNameWithoutExtension(GFF)
                Dim Fna As String = (From path As String
                                     ...
                                     Let FNA_ID As String = IO.Path.GetFileNameWithoutExtension(path)
