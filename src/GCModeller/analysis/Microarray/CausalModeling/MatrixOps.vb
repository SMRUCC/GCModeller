...
Public Module MatrixOps
    ...
    Public Function Inverse(a As Double(,)) As Double(,)
        ...
        For col = 0 To n - 1
            ' 寻找主元
            Dim maxRow = col
            Dim maxVal = Math.Abs(aug(col, col))
            ...
            If Math.Abs(pivot) < 1e-14 Then Throw New Exception("矩阵奇异，无法求逆")
            For j = 0 To 2 * n - 1
                aug(col, j) /= pivot
            Next
    ...
    Public Function Determinant(a As Double(,)) As Double
        ...
        For k = 0 To n - 1
            ...
            If Math.Abs(lu(k, k)) < 1e-14 Then Return 0.0
    ...
    Public Function JacobiEigen(a As Double(,)) As (eigenvalues As Double(), eigenvectors As Double(,))
        ...
        For iter = 1 To maxIter
            ...
            For i = 0 To n - 2
                For j = i + 1 To n - 1
                    If Math.Abs(mat(i, j)) > maxOff Then
                        maxOff = Math.Abs(mat(i, j))
                        p = i
                        q = j
                    End If
                Next
            ...
            Dim theta As Double
            If Math.Abs(app - aqq) < 1e-30 Then
                theta = Math.PI / 4
            Else
    ...
    Public Function Trace(a As Double(,)) As Double
        Dim n = Math.Min(a.GetLength(0), a.GetLength(1))
        Dim tr = 0.0
        For i = 0 To n - 1
            tr += a(i, i)
        Next
