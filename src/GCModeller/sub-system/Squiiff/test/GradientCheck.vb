...
'''
''' 校验方式：固定随机权重 <c>w</c>，取标量目标
''' <c>L = Σ (w ⊙ out)</c>，于是 <c>dL/dOut = w</c>，用各模块的 <c>Backward(w)</c> 得到解析梯度，
''' 再对参数的每个采样元素做 <c>(L(p+h) − L(p−h)) / 2h</c> 与解析值比对。
''' </summary>
Module GradientCheck
    ...
    Public Function Run(Optional seed As Integer = 20240920, Optional tolerance As Double = 0.00001) As Boolean
        ...
        Dim backend = Tensor.computeKernel.Name

        Console.WriteLine()
        Console.WriteLine(New String("="c, 78))
        Console.WriteLine("  有限差分梯度校验（解析反向 vs 中心差分数值梯度）")
        Console.WriteLine($"  张量计算后端 = {backend}    容差 = {tolerance:E1}")
        Console.WriteLine(New String("="c, 78))

        Dim pass As Boolean = True

        pass = CheckLayer("Linear", New Linear("gc.linear", 5, 4), {6, 5}, rng, tolerance) And pass
        pass = CheckLayer("ActivationLayer(ReLU)", New ActivationLayer("gc.relu", ActivationKind.ReLU), {6, 5}, rng, tolerance) And pass
        pass = CheckLayer("ActivationLayer(SiLU)", New ActivationLayer("gc.silu", ActivationKind.SiLU), {6, 5}, rng, tolerance) And pass
        pass = CheckLayer("ActivationLayer(Tanh)", New ActivationLayer("gc.tanh", ActivationKind.Tanh), {6, 5}, rng, tolerance) And pass
        pass = CheckLayer("ActivationLayer(Sigmoid)", New ActivationLayer("gc.sigmoid", ActivationKind.Sigmoid), {6, 5}, rng, tolerance) And pass
...
#Region "校验引擎"
    ...
    Private Function CheckLayer(name As String, layer As LayerModule, inputShape As Integer(),
        ...
        Return NumericCheck(name, layer.Parameters, dx, {x},
                            Function() layer.Forward(x, training:=True), rng, tolerance)
    End Function
    ...
    Private Function CheckConditioned(name As String, norm As ConditionalBatchNorm,
        ...
        Return NumericCheck(name, norm.Parameters, dx, {x, cond},
                            Function() norm.Forward(x, cond, training:=True), rng, tolerance,
                            {dCond})
    End Function
    ...
    Private Function CheckResidual(name As String, block As ResidualBlock,
        ...
        Return NumericCheck(name, block.Parameters, dx, {x, cond},
                            Function() block.Forward(x, cond, training:=True), rng, tolerance,
                            {dCond})
    End Function

    ''' <summary>
    ''' 数值梯度校验引擎：对标量损失 <c>L = Σ(w ⊙ out)</c> 校验参数梯度与输入梯度。
    ''' </summary>
    ''' <param name="parameters">待校验的可训练参数。</param>
    ''' <param name="dFirstInput">解析的"对第 0 个输入"的梯度。</param>
    ''' <param name="inputs">参与校验的输入张量（第 0 个通常为主输入）。</param>
    ''' <param name="forward">按当前参数/输入值重新计算输出的函数。</param>
    ''' <param name="dOtherInputs">其余输入的解析梯度（与 <paramref name="inputs"/> 的第 1..n 项对应）。</param>
    Private Function NumericCheck(name As String, parameters As IEnumerable(Of Parameter),
                                  dFirstInput As Tensor, inputs As Tensor(),
                                  forward As Func(Of Tensor), rng As Random, tolerance As Double,
                                  Optional dOtherInputs As Tensor() = Nothing) As Boolean
        ' 固定随机权重 w，使 dL/dOut = w
        Dim out = forward()
        Dim w = TensorUtil.StandardNormal(out.Shape, rng)
        Dim lossFunc As Func(Of Double) = Function() WeightedSum(forward(), w)

        ' 1) 先固化解析梯度（后续数值扰动会重新前向，必须避免读到被覆盖的状态）
        Dim analyticParams As New List(Of Parameter)
        Dim analyticValues As New List(Of Double())
        For Each p In parameters
            If p Is Nothing Then Continue For
            analyticParams.Add(p)
            analyticValues.Add(CType(p.Gradient.Data.Clone(), Double()))
        Next
        ...
        ' 3) 输入梯度
        Dim inputGradients As New List(Of Tensor)
        inputGradients.Add(dFirstInput)
        If dOtherInputs IsNot Nothing Then inputGradients.AddRange(dOtherInputs)

        For t As Integer = 0 To std.Min(inputs.Length, inputGradients.Count) - 1
            Dim inputTensor = inputs(t)
            Dim analytic = inputGradients(t)
            ...
            For Each index In SampleIndices(inputTensor.Length, 8, rng)
                ...
                Dim value = analytic.Data(index)
                Dim error1 = RelativeError(value, numeric)
                maxError = std.Max(maxError, error1)
                If error1 > tolerance Then
                    ok = False
                    Console.WriteLine($"    [FAIL] {name} · dInput{t}[{index}]  解析={value:E3}  数值={numeric:E3}  误差={error1:E2}")
                End If
            Next
        Next
