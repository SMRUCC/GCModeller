...
Module Program
    ...
    Sub Main(args As String())
        ...
        Console.WriteLine("      代谢重编程：呼吸链 CYTBO3 / ATPS4r 与 PDH 通量大幅下降，")
        Console.WriteLine("      乳酸（LDH_L）与乙酸（ACKr）支路通量成倍上升，胞内 NADH 累积")
        Console.WriteLine("      —— 与真实的有氧 → 无氧切换一致。")
        Console.WriteLine("   2) 单酶敲除（CYTBO3 / ATPS4r 设为 0）属于训练时从未见过的扰动，")
        Console.WriteLine("      被敲除反应的通量立刻归零，下游也给出了同向的重编程响应")
        Console.WriteLine("      （发酵支路上升、PDH 下降），但幅度明显弱于溶氧切换。")
