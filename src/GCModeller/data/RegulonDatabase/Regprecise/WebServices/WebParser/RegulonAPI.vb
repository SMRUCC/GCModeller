...
Namespace Regprecise
    ...
    Public Module RegulonAPI
        ...
        Public Function Reconstruct(mappings As String, Regprecise As String, DOOR As DOOR) As BacteriaGenome
            Dim bbh = mappings.LoadCsv(Of NCBI.Extensions.LocalBLAST.Application.BBH.BiDirectionalBesthit)
        ...
        Public Function BuildMapHash(mappings As IEnumerable(Of NCBI.Extensions.LocalBLAST.Application.BBH.BiDirectionalBesthit)) _
            As Dictionary(Of String, NCBI.Extensions.LocalBLAST.Application.BBH.BiDirectionalBesthit())

            Dim mappingHash = (From x As NCBI.Extensions.LocalBLAST.Application.BBH.BiDirectionalBesthit In mappings
                               Where x.Matched
                               Select x
        ...
        <ExportAPI("Regulon.Reconstruct")>
        <Extension>
        Public Function Reconstruct(mappings As Dictionary(Of String, NCBI.Extensions.LocalBLAST.Application.BBH.BiDirectionalBesthit()),
                                    Regprecise As BacteriaGenome,
                                    Operons As DOOR) As Regulator()
            Dim LQuery As Regulator() = (From x As Regulator
                                         In Regprecise.Regulons.Regulators
                                         Select mappings.Reconstruct(regulon:=x, DOOR:=Operons)).ToArray.MatrixToVector
            Return LQuery
        End Function
