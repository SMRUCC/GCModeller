...
#End Region
        ...
        Public Function GetVector() As Double()
            Return New Double() {
                ...
                z_substrate,           ' 底物净电荷数
                Ki_sub,                ' 底物抑制常数 (M)
                Ki_prod_comp,          ' 产物竞争抑制常数 (M)
                Ki_prod_uncomp,        ' 产物非竞争抑制常数 (M)
                If(RequiresCofactor, 1, 0),' 是否需要辅因子
                Ka_cofactor,           ' 辅因子激活常数 (M)
                n_cofactor,            ' 辅因子结合协同性 (Hill系数)
                crowding_sensitivity,  ' 拥挤敏感度 (0~1)
                viscosity_sensitivity, ' 粘度敏感度 (0~1), 1为纯扩散控制
                alpha_temp_pH,         ' 温度-pH交互作用系数
                beta_ion_pH,           ' 离子强度-pH交互作用系数
                tau_temp,              ' 温度适应时间常数 (秒)
                tau_pH                 ' pH适应时间常数 (秒)
            }
        End Function
        ...
        Public Shared Function FromVector(v As Double()) As EnzymeCharacteristics
            Return New EnzymeCharacteristics With {
                .T_ref = v(0),                  ' 参考温度 (K)
                .T_opt = v(1),                  ' 最适温度 (K)
                .Ea = v(2),                     ' kcat 活化能 (J/mol)
                .DeltaH_bind = v(3),            ' Km 结合焓 (J/mol)
                .T_melt = v(4),                 ' 酶熔化温度 (K)
                .k_denat = v(5),                ' 失活陡峭度系数
                .pH_opt = v(6),                 ' 最适 pH
                .pKa1 = v(7),                   ' 催化残基质子解离常数
                .pKa2 = v(8),                   ' 催化残基去质子解离常数
                .pH_width = v(9),               ' pH活性曲线宽度
                .I_opt = v(10),                 ' 最适离子强度 (M)
                .k_ion = v(11),                 ' 离子强度影响系数
                .z_enzyme = v(12),              ' 酶净电荷数
                .z_substrate = v(13),           ' 底物净电荷数
                .Ki_sub = v(14),                ' 底物抑制常数 (M)
                .Ki_prod_comp = v(15),          ' 产物竞争抑制常数 (M)
                .Ki_prod_uncomp = v(16),        ' 产物非竞争抑制常数 (M)
                .RequiresCofactor = v(17) <> 0, ' 是否需要辅因子
                .Ka_cofactor = v(18),           ' 辅因子激活常数 (M)
                .n_cofactor = v(19),            ' 辅因子结合协同性 (Hill系数)
                .crowding_sensitivity = v(20),  ' 拥挤敏感度 (0~1)
                .viscosity_sensitivity = v(21), ' 粘度敏感度 (0~1), 1为纯扩散控制
                .alpha_temp_pH = v(22),         ' 温度-pH交互作用系数
                .beta_ion_pH = v(23),           ' 离子强度-pH交互作用系数
