...
# Session
_namespace: [SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings](N-SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings.html)_
...
#### Finallize
```csharp
SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
SMRUCC.genomics.Interops.NBCR.MEME_Suite.TMoD.Settings.Session.TryGetShoalShellBin
```
