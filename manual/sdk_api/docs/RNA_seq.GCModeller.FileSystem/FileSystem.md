# FileSystem
_namespace: [RNA_seq.GCModeller.FileSystem](./index.md)_
...
#### GetLocalblast
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.GetLocalblast
```
...
#### GetMotifLDM
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.GetMotifLDM(System.String)
```
<RegpreciseRoot>/MEME/MAST_LDM/

#### GetPfamDb
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.GetPfamDb(System.String)
```
...
#### GetRegulations
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.GetRegulations
```
regulations.xml文件在GCModeller数据库之中的位置

#### GetRepositoryRoot
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.GetRepositoryRoot
```
The root directory for stores the GCModeller database such as fasta sequence for annotation.

#### IsRepositoryNullOrEmpty
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.IsRepositoryNullOrEmpty
```
配置文件之中是否包含有GCModeller数据库的位置的路径参数

#### TryGetSource
```csharp
RNA_seq.GCModeller.FileSystem.FileSystem.TryGetSource(System.String,System.Func{System.String})
```
